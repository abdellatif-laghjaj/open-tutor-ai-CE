...
<script lang="ts">
	...
	let chatCreatedHandler: EventListener;
	...
	onMount(() => {
		if (browser) {
			...
			try {
				...
				if (pendingSupportData) {
					...
					if (currentTime - supportTimestamp < MAX_SUPPORT_AGE_MS) {
						...
						console.log(
							'Found valid pendingSupportId:',
							pendingSupportId,
							'created',
							Math.round((currentTime - supportTimestamp) / 1000),
							'seconds ago'
						);
					} else {
			...
			// Add global event listener for chat creation — store reference for cleanup
			chatCreatedHandler = ((event: CustomEvent) => {
				const newChatId = event.detail?.chatId;
				if (newChatId && pendingSupportId) {
					updateSupportWithChatId(pendingSupportId, newChatId);
				}
			}) as EventListener;
			window.openTutorEvents.addEventListener('chatCreated', chatCreatedHandler);
	...
	onDestroy(() => {
		if (browser) {
			if (chatCreatedHandler) {
				window.openTutorEvents.removeEventListener('chatCreated', chatCreatedHandler);
			}
	...
	async function saveSupportToDatabase() {
		...
		try {
			...
			if (supportResponse && supportResponse.id) {
				...
				if (uploadedFiles.length > 0) {
					try {
						...
						toast.error(
							$i18n.t(
								`Support saved, but file upload failed: ${fileError?.message || 'Unknown error'}`
							)
						);
					}
				}
				...
				if (browser) {
					window.openTutorEvents.dispatchEvent(
						new CustomEvent('supportCreated', {
							detail: {
			...
			toast.error(
				`${$i18n.t('An error occurred')}: ${error?.message || $i18n.t('Failed to process your request. Please try again.')}`
			);
		} finally {
			isSubmitting = false;
		}
						...
						<div class="space-y-8 step-content-enter">
							<div>
								...
								<div class="mb-8">
									...
									<p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
										{$i18n.t(
											'A brief overview helps us tailor the learning experience to your needs'
										)}
									</p>
								</div>
							</div>
								...
								>
									<div class="flex items-center justify-between mb-3">
										...
										<div class="relative group">
											...
											>
												{$i18n.t(
													'AI can help you craft personalized learning objectives based on your interests and goals'
												)}
											</div>
										</div>
									</div>
									...
									<p class="text-sm text-gray-600 dark:text-gray-400 mb-2">
										{$i18n.t(
											'Be specific about what you hope to achieve by the end of this support'
										)}
									</p>
						...
						<div class="space-y-8 step-content-enter">
							<div>
								...
								<p class="text-gray-600 dark:text-gray-400 mb-6">
									{$i18n.t(
										'Select the appropriate learning level for this material to ensure the content matches your needs'
									)}
									<span class="text-red-500 ml-1">*</span>
								</p>
									...
									<div class="p-0">
										...
										<div class="px-6 py-5 bg-gray-50 dark:bg-gray-700">
											<div class="flex items-center text-gray-700 dark:text-gray-300">
												...
												<p class="text-sm">
													{$i18n.t(
														'Click "Start Learning" below to create your support and begin your personalized learning experience.'
													)}
												</p>
											</div>
										</div>
		...
		{:else}
			...
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center animate-fadeIn">
				...
				<h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">
					{$i18n.t('Support Created!')}
				</h2>
				<p class="text-gray-600 dark:text-gray-300 mb-8 max-w-md mx-auto">
					{$i18n.t(
						'Your support has been successfully created. Get ready for a personalized learning experience!'
					)}
				</p>
